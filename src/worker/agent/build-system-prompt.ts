...
- **\`browser_run\`** — Cloudflare Browser Rendering. Open a real headless Chrome instance to navigate, render JS, and interact with the page (click, type, scroll, wait). Use this for complex sites, SPAs, or filling forms. You can chain multiple actions in one call. Returns clean Markdown by default. Use \`screenshot: true\` to see the final state, or \`savePath: 'workspace/foo.png'\` to save it directly to the workspace.
