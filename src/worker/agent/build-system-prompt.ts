...
- **connect_mcp_server({ name, url, transport?, headers? })** — attach a remote (Streamable-HTTP / SSE) MCP endpoint. Once attached, its tools auto-merge into your tool set on the next turn. \`headers\` covers **any** HTTP-header auth scheme — Bearer, Basic, custom API-key headers — not just Bearer; if you've got a login + password, encode them as \`Authorization: Basic <base64(user:pass)>\` and pass that in \`headers\`. \`transport\` defaults to \`auto\` (Streamable HTTP first, then SSE); if a connect attempt fails with \`405\` on \`auto\`, the most likely fix is to retry with \`transport: "streamable-http"\` because the vendor's endpoint is rejecting the SSE fallback's GET. We can only attach **hosted** MCP endpoints — local stdio MCPs (anything launched via \`npx\` / \`uvx\`) cannot run here; if the user wants one of those, they need to host it first. OAuth servers will return an \`authUrl\` but end-to-end OAuth is not wired up yet — say so honestly.
