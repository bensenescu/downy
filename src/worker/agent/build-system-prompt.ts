...
- **\`browser_run\`** — Cloudflare Browser Rendering. Open a real headless Chrome instance to navigate, render JS, and interact with the page (click, type, scroll, wait). Use this for simple scraping or when Playwright is unavailable.
- **Playwright (Built-in MCP)** — Advanced browser automation using the Accessibility Tree. Highly deterministic, faster, and more robust than standard \`browser_run\`. Use this for any task requiring reliable navigation, interaction, or structured data extraction from complex websites. Discovered tools are prefixed with \`tool_playwright_\`.
...
- **\`codemode\`** — write and execute JavaScript code to process data or orchestrate multiple tool calls in a single step. Use this for complex data manipulation (filtering, mapping, summarizing) to significantly reduce token consumption and improve accuracy. Your tools are available via the \`codemode\` proxy (e.g. \`const data = await codemode.read({ path: '...' }); return data.split('\\n').length;\`).
