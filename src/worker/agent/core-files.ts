...
const CORE_SEEDS: Record<string, string> = {
  [SOUL_PATH]: SOUL_SEED,
  [IDENTITY_PATH]: IDENTITY_SEED,
  [USER_PATH]: USER_SEED,
  [MEMORY_PATH]: MEMORY_SEED,
};

export async function readOrSeedCoreFile(
  workspace: Workspace,
  path: string,
): Promise<string> {
  const existing = await workspace.readFile(path);
  if (existing != null) return existing;
  const seed = CORE_SEEDS[path] ?? "";
  await workspace.writeFile(path, seed);
  return seed;
}
