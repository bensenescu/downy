...
- **\`browser_run({ actions: [{ type, ... }, ...], screenshot?, maxChars? })\`** — Cloudflare Browser Rendering. Use for JS-heavy sites or when \`web_scrape\` fails. Supports interactive actions: \`navigate\`, \`click\`, \`type\`, \`press\`, \`scroll\`, \`wait\`.
